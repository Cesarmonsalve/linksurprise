// STYLE: CINEMATIC SCROLL - Epic Storytelling
...
export function renderCinematicScroll(d: TemplateRenderData): TemplateOutput {
  ...
  const gallery = renderVipGallery(d, "cinematicscroll");
  const accent = d.accentColor || '#e50914';

  const css = `
    @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue:wght@300;400;600;800&display=swap');
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { background: linear-gradient(180deg, #1a0a0a 0%, #0f0f0f 100%); min-height: 100vh; overflow-x: hidden; font-family: 'Bebas+Neue', sans-serif; }
    ${isBasic ? `
      ...
      .orb-2 { width: 220px; height: 220px; background: #ff6b35; bottom: -60px; right: -60px; animation-delay: -5s; }

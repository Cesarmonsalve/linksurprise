// STYLE: ZEN MINIMAL - Peaceful Simplicity
...
export function renderZenMinimal(d: TemplateRenderData): TemplateOutput {
  ...
  const accent = d.accentColor || '#78909c';

  const css = `
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@300;400;600;800&display=swap');
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { background: linear-gradient(180deg, #f5f7fa 0%, #e4e8eb 100%); min-height: 100vh; overflow-x: hidden; font-family: 'Noto+Sans+JP', sans-serif; }
    ${isBasic ? `
      ...
      .orb-2 { width: 220px; height: 220px; background: #b0bec5; bottom: -60px; right: -60px; animation-delay: -5s; }
